...
class RexModelTrainer(Trainer):
    ...
    @property
    def tokenizer(self):
        """兼容性处理：使用 processing_class 代替已废弃的 tokenizer 属性"""
    ...
    def prediction_step(
        ...
        if self.tokenizer.additional_special_tokens[2] in text and pred_info_list:
            ...
            # 获取 top_k 索引
            sorted_indices = sorted(range(len(keys_prob)), key=lambda i: keys_prob[i], reverse=True)[:top_k]
            top_k_results = []
            for idx in sorted_indices:
                result_items = pred_info_list[idx]
                # 保留置信度信息
                avg_confidence = keys_prob[idx]
                for item in result_items:
                    item['avg_confidence'] = avg_confidence
                top_k_results.append(result_items)
            pred_info_list = top_k_results
    ...
    def prompt_loop(self, model, text, level_hint_map, pred_info_list, legal_output_type_list, blank=False):
            ...
            if data_type == 'no_cls':
                ...
                if data_type == 'cls':
                    if len(pred_info) == 1:
                        ...
                        # 将所有标签的结果都添加到候选列表
                        for keys, probs in prob_sum.items():
                            pred_info_cands.append((keys, probs))
        ...
        if data_type == 'cls' and pred_info_cands:
            # 去重：如果同一个标签出现多次，保留概率最高的
            seen_keys = {}
            for cand_keys, cand_probs in pred_info_cands:
                prob_sum = sum(cand_probs)
                if cand_keys not in seen_keys or prob_sum > seen_keys[cand_keys][1]:
                    seen_keys[cand_keys] = (cand_probs, prob_sum)
            
            # 按概率排序后添加所有标签的结果
            sorted_cands = sorted(seen_keys.items(), key=lambda x: x[1][1], reverse=True)
            for cand_keys, (cand_probs, _) in sorted_cands:
                keys = cand_keys.split('[INFOKEY]')
                info_list = []
                for i, key in enumerate(keys):
                    info_list.append({'type': key, 'span': cls_token, 'offset': [0, len(cls_token)], 'confidence': cand_probs[i]})
                pred_info_list.append(info_list)
                # if len(pred_info) == 1:
